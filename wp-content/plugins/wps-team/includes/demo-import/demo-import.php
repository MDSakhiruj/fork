...
class Demo_Import
{
    ...
    {
        // plugin()->notice->manager->force_dismiss_admin_notice( self::demo_import_key() );
        
        if ( get_option( self::$key . '_dummy_post_data_created' ) !== false || get_transient( self::$key . '_dummy_post_data_creating' ) !== false ) {
            $message_202 = __( 'Dummy team members already imported', 'wpspeedo-team' );
            if ( wp_doing_ajax() ) {
        ...
        $message = __( 'Dummy team members imported', 'wpspeedo-team' );
        if ( wp_doing_ajax() ) {
    ...
    public function remove_demo_data()
    {
        ...
        $message = __( 'Dummy team members deleted', 'wpspeedo-team' );
        if ( wp_doing_ajax() ) {
            wp_send_json_success( $message, 200 );
        }
        return [
    ...
    function admin_notice()
    {
        ...
        if ( !plugin()->notice->manager->is_admin_notice_active( self::demo_import_key() ) ) {
            return;
        }
        if ( get_current_screen()->id == Utils::post_type_name() . '_page_wps-team' ) {
            return;
        }
        if ( get_current_screen()->id == Utils::post_type_name() . '_page_wps-team-account' ) {
            return;
        }
        ?>
        <div class="wpspeedo--notice notice is-dismissible" data-dismissible="<?php 
        echo  self::demo_import_key() ;
        ?>">

            <div class="wpspeedo--notice-inner">

                <div class="wpspeedo--notice-col logo-area">
                    <img src="<?php 
        echo  WPS_TEAM_URL . 'images/logo.svg' ;
        ?>" alt="">
                    <p>A Product of wpspeedo.com</p>
                </div>
    
                <div class="wpspeedo--notice-col content-area">
    
                    <h3 class="notice-title">Import Demo Data!</h3>

                    <p>Install the demo data quickly by one click & kick start with <b>WPS Team</b> plugin.<br>You can remove the demo data anytime you want.</p>
    
                    <p class="wpspeedo--notice-actions">
                        <a href="<?php 
        echo  admin_url( 'edit.php?post_type=' . Utils::post_type_name() . '&page=wps-team#/tools/demo-import' ) ;
        ?>" class="button button-primary" style="margin-right: 20px;">Import Demo Data</a>
                        <a href="#" class="dismiss-this"><?php 
        _e( "Don't show this message again", 'wpspeedo-team' );
        ?></a>
                    </p>
