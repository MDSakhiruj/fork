...
class Initialize {
    ...
    public function fail_php_version() {
        $message = '<strong>WPS Team</strong> ' . sprintf( esc_html__( 'plugin requires PHP version %s+, plugin is currently NOT RUNNING.', 'wpspeedo-team' ), '5.6' );
        echo wp_kses_post( sprintf( '<div class="error">%s</div>', wpautop( $message ) ) );
    }
    
    public function fail_wp_version() {
        $message = '<strong>WPS Team</strong> ' . sprintf( esc_html__( 'plugin requires WordPress version %s+. Because you are using an earlier version, the plugin is currently NOT RUNNING.', 'wpspeedo-team' ), '5.0' );
        echo wp_kses_post( sprintf( '<div class="error">%s</div>', wpautop( $message ) ) );
    }
